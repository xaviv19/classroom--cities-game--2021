...
export class PlaysNNCardsIntoThePile extends AbstractPostLineStep {
  constructor() {
    super(
      /_([^_]+)_ plays (\d) _([^_]+)_ and (\d) _([^_]+)_ _([^_]+)_ cards into the _([^_]+)_ pile/i
    );
  }

  async runMatch(_: any, match: string[]) {
    const [, player, count1, name1, count2, name2, type, pile] = match;
